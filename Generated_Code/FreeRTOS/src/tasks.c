...
#include "UTIL.h" /* interface to utility because used for safe string routines */ /* << EST */
#endif
...
#else /* << EST */
	      UTIL_strcat((uint8_t*)pcWriteBuffer, bufSize, (const unsigned char*)pxTaskStatusArray[ x ].pcTaskName);
	      UTIL_strcat((uint8_t*)pcWriteBuffer, bufSize, (const unsigned char*)"\t\t");
	      UTIL_chcat((uint8_t*)pcWriteBuffer, bufSize, (unsigned char)cStatus);
	      UTIL_chcat((uint8_t*)pcWriteBuffer, bufSize, (unsigned char)'\t');
	      UTIL_strcatNum32u((uint8_t*)pcWriteBuffer, bufSize, pxTaskStatusArray[ x ].uxCurrentPriority);
	      UTIL_chcat((uint8_t*)pcWriteBuffer, bufSize, (unsigned char)'\t');
	      UTIL_strcatNum32u((uint8_t*)pcWriteBuffer, bufSize, pxTaskStatusArray[ x ].usStackHighWaterMark);
	      UTIL_chcat((uint8_t*)pcWriteBuffer, bufSize, (unsigned char)'\t');
	      UTIL_strcatNum32u((uint8_t*)pcWriteBuffer, bufSize, pxTaskStatusArray[ x ].xTaskNumber);
	      UTIL_strcat((uint8_t*)pcWriteBuffer, bufSize, (const unsigned char*)"\r\n");
#endif
			}
		...
		*pcWriteBuffer = 0x00;
		...
		if( pxTaskStatusArray != NULL )
		{
			...
			if( ulTotalTime > 0 )
			{
              ...
              UTIL_strcat((uint8_t*)pcWriteBuffer, bufSize, (const unsigned char*)pxTaskStatusArray[ x ].pcTaskName);
              UTIL_strcat((uint8_t*)pcWriteBuffer, bufSize, (const unsigned char*)"\t\t");
              UTIL_strcatNum32u((uint8_t*)pcWriteBuffer, bufSize, pxTaskStatusArray[ x ].ulRunTimeCounter);
              UTIL_strcat((uint8_t*)pcWriteBuffer, bufSize, (const unsigned char*)"\t\t");
              UTIL_strcatNum32u((uint8_t*)pcWriteBuffer, bufSize, ulStatsAsPercentage);
              UTIL_strcat((uint8_t*)pcWriteBuffer, bufSize, (const unsigned char*)"%\r\n");
#endif
						}
						#endif
              ...
              UTIL_strcat((uint8_t*)pcWriteBuffer, bufSize, (const unsigned char*)"\t\t");
              UTIL_strcatNum32u((uint8_t*)pcWriteBuffer, bufSize, pxTaskStatusArray[ x ].ulRunTimeCounter);
              UTIL_strcat((uint8_t*)pcWriteBuffer, bufSize, (const unsigned char*)"\t\t<1%\r\n");
#endif
						}
						#endif
					}
