/*
 * crc.c
 *
 *  Created on: 13 juin 2014
 *      Author: OrpheoXlite
 */
#include "crc.h"

/**
 *  CRC8 array
 *  POLY = 0x31
 */
static const uint8_t CRC8_ARRAY[256]={
        };

/**
 *  METHOD:
 *  @bref
 *      Calculate by Byte the CRC code, using a table of CRC code.
 *  @para
 *      *data
 *          - Adress of the first data frame
 *      len
 *          - Length of the data frame in byte
 *  @return
 *      crc
 *          - The calculated CRC code
 */
uint8_t GetCRC8(const uint8_t *data, uint32_t len)
{
    uint8_t crc=0;
    while (len--!=0) {
        crc =CRC8_ARRAY[crc ^ *data++];
    }
    return crc;
}

#ifdef GENERATE_CRC8_ARRAY
/**
 * This part can enabled only if you want to generate a crc8 array.
 */
/**
 *  METHOD: CRC8_Table_Gen
 *  @bref
 *      Generate a CRC8 code table using generator polynomial.
 *  @para
 *      *ptr
 *          - Adress of the first data frame
 *      len
 *          - Length of the data frame in byte
 *  @return
 *      crc
 *          - The calculated CRC code
 */
void CRC8_Table_Gen(uint8_t *CRC8_table, uint8_t poly)
{
    uint8_t crc_8;
    uint32_t j;
    uint32_t i;

    for ( i = 0; i < 256; i++ ) {
        crc_8 = i;
        for (j = 8; j > 0; j--) {
            if(crc_8&0x80)
                crc_8=(crc_8<<1)^poly;
            else
                crc_8<<=1;
        }
        CRC8_table[i] = crc_8;
    }
}
#endif  // GENERATE_CRC8_ARRAY
